...
scIB_knit_table <- function(
  ...
  if(length(ind_circle) > 0){
    ...
    temp = unlist(lapply(colnames(dat_mat), 
                         function(x) rep(x, nrow(dat_mat))))
    circle_data <- data.frame(label = temp, 
  ...
  rect_data <- data.frame(label = temp,
                          method = rep(row_info$id, ncol(dat_mat)),
                          value = as.vector(unlist(dat_mat)),
  ...
  x_min_score <-  ifelse(atac, minimum_x + 12, minimum_x + 18)
  ...
  leg_min_x <- x_min_ranking-1
  rank_groups <- as.character(column_info[column_info$geom == "bar" | column_info$geom == "circle", "group"])
  ...
  if(usability){
    ...
    rank_minimum_x <- list("Overall" = leg_min_x,
                           "Biological Accuracy" = leg_min_x + 1,
                           "Batch Removal" = leg_min_x + 2,
                           "Artifact Removal" = leg_min_x + 3,
                           "Differential Reduction" = leg_min_x + 4,
                           "Scalability" = leg_min_x + 5)
    leg_max_x <- leg_min_x+5
  }
