...
#'
TMM_FN = function(ei) {
  size_fac = edgeR::calcNormFactors(ei, method = "TMM")
...
DESEQ_FN = function(ei) {
  size_fac = edgeR::calcNormFactors(ei, method = "RLE")
  scales = (colSums(ei) * size_fac)
  eo = t(t(ei) * mean(scales) / scales)
  return(eo)
}
...
#'
FQT_FN = function(ei) {
  # eo = limma::normalizeQuantileRank.matrix(ei, ties = TRUE) # this function is only in aroma.light not limma
  eo = limma::normalizeQuantiles(ei, ties = TRUE)
  return(eo)
}
